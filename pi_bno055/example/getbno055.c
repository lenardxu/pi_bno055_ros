...
int verbose = 0;
int outflag = 0;
int argflag = 0; // 1 dump, 2 reset, 3 load calib, 4 write calib, 5 close I2C bus
char g_range[4] = {0};
char opr_mode[9] = {0};
char pwr_mode[8] = {0};
char unit_mode[5] = {0};
 ...
 * ------------------------------------------------------------ */
void usage() {
   static char const usage[] = "Usage: getbno055 [-a hex i2c-addr] [-c 2g|4g|8g|16g] [-m <opr_mode>] [-t acc|gyr|mag|eul|qua|lin|gra|inf|imu|imu_sp|cal|con] [-u rosa | rosw] [-r] [-q] [-w calfile] [-l calfile] [-o htmlfile] [-v]\n\
\n\
...
void parseargs(int argc, char* argv[]) {
   ...
   while ((arg = (int) getopt (argc, argv, "a:b:c:dm:p:u:rqt:l:w:o:hv")) != -1) {
      switch (arg) {
         ...
         // arg -c + acc_range (now only the option of acc_range), type: string
         case 'c':
            if(verbose == 1) printf("Debug: arg -c, value %s\n", optarg);
            if (strlen(optarg) >= sizeof(g_range)) {
                printf("Error: invalid acc_range argument.\n");
                exit(-1);
            }
            strncpy(g_range, optarg, sizeof(g_range));
            break;
         ...
         // arg -u sets unit selection (now only the option of ros-specific unit selection), type: string
         case 'u':
            if(verbose==1) printf("Debug: arg -u, value %s\n", optarg);
            if (strlen(optarg) >= sizeof(unit_mode)) {
                printf("Error: invalid unit_mode argument.\n");
                exit(-1);
            }
            strncpy(unit_mode, optarg, sizeof(unit_mode));
            break;
         ...
         // arg -q
         // optional, close the I2C bus communication with sensor
         case 'q':
            if(verbose==1) printf("Debug: arg -q, value %s\n", optarg);
            argflag = 5;
            break;
         ...
         case 't':
            ...
            if (strlen(optarg) != 3 && strlen(optarg) != 6) {
...
int main(int argc, char *argv[]) {
   if(verbose == 1) printf("%s pi_bno055 Version: %d.%d\n", argv[0], pi_bno055_VERSION_MAJOR, pi_bno055_VERSION_MINOR);
    ...
    * ----------------------------------------------------------- */
    ...
    if(argflag == 5) {
        close_i2cbus();
        if(verbose==1) printf("Debug: I2C bus communication is closed");
        exit(0);
    }

    /* ----------------------------------------------------------- *
    *  "-u" set the unit selection incl. data output format.      *
    * ----------------------------------------------------------- */
    if(strlen(unit_mode) > 0) {
        unit_sel_t unitsel;
        if(strcmp(unit_mode, "rosa") == 0) unitsel = ros_imu_andr;
        else if(strcmp(unit_mode, "rosw")  == 0) unitsel = ros_imu_win;
        else {
            printf("Error: invalid unit mode %s.\n", unit_mode);
            exit(-1);
        }

        if(unitsel == get_unit()) {
            if(verbose==1) printf("Debug: Sensor already in unit selection %s [0x%02X].\n", unit_mode, unitsel);
            exit(0);
        }

        res = set_unit(unitsel);
        if(res != 0) {
            printf("Error: could not set unit selection %s [0x%02X].\n", unit_mode, unitsel);
            exit(-1);
        }
    }
    ...
    * ----------------------------------------------------------- */
   if(strlen(opr_mode) > 0) {  // TODO refer to "3.3 Operation Modes, especially 3.3.3 Fusion modes" whenever confused with operation mode
      opmode_t newmode;
    ...
    /* -------------------------------------------------------------- *
     *  "-c" set the sensor config param (now only g range for acc)   *
     * -------------------------------------------------------------- */
    if(strlen(g_range) > 0) {
        g_range_t grange;
        if(strcmp(g_range, "2g") == 0) grange = g2;
        else if(strcmp(g_range, "4g") == 0) grange = g4;
        else if(strcmp(g_range, "8g") == 0) grange = g8;
        else if(strcmp(g_range, "16g") == 0) grange = g16;
        else {
            printf("Error: invalid sensor config (g range) %s.\n", g_range);
            exit(-1);
        }

        res = set_acc_conf(grange);
        if(res != 0) {
            printf("Error: could not set g_range of sensor config as %s [0x%02X].\n", g_range, grange);
            exit(-1);
        }
        exit(0);
    }
    ...
    * ----------------------------------------------------------- */
    if(argflag == 4) {
       ...
       * -------------------------------------------------------- */
      char calfile_path[256] = PROJECT_ROOT_DIR;
      strcat(calfile_path, "/");
      strcat(calfile_path, calfile);
//      if(verbose==1) printf("Debug: the full calibration file path: %s", calfile_path);
      if(bnoc.scal_st == 3) save_cal(calfile_path);
    ...
    * ----------------------------------------------------------- */
   if(strcmp(datatype, "eul") == 0) {  // TODO see function print_mode(int mode) in i2c_bno055.c for concrete modes (totally 13 modes) & "3.3 Operation Modes, especially 3.3.3 Fusion modes"
       ...
       * ----------------------------------------------------------- */
      printf("EUL %3.4f %3.4f %3.4f\n", bnod.eul_head, bnod.eul_roll, bnod.eul_pitc);  // TODO to learn about the format specifier in float output: https://codzify.com/C/formatSpecifiers
  ...
  /* ----------------------------------------------------------- *
   ...
   if(strcmp(datatype, "con") == 0) {
      ...
      struct bnogyr bnod_gyr;
      struct bnoeul bnod_eul;
      struct bnolin bnod_lin;
      unsigned int cnt = 0;
      double mean_time_taken = 0.;
      /* ----------------------------------------------------------- *
       * print the formatted output string to stdout (Example below) *
       * EUL 66.06 -3.00 -15.56 (EUL H R P in Degrees)               *
       * ----------------------------------------------------------- */
      while(1){
        ...
        res = get_gyr(&bnod_gyr);
        if(res != 0) {
            printf("Error: Cannot read Angular rate data.\n");
            continue;
        }
        res = get_eul(&bnod_eul);
        if(res != 0) {
           printf("Error: Cannot read Euler orientation data.\n");
           continue;
        }
        res = get_lin(&bnod_lin);
        if(res != 0) {
        ...
        printf("GYR %3.2f %3.2f %3.2f; EUL %3.2f %3.2f %3.2f; LIN %3.2f %3.2f %3.2f\n",
                    bnod_gyr.gdata_x, bnod_gyr.gdata_y, bnod_gyr.gdata_z,
                    bnod_eul.eul_head, bnod_eul.eul_roll, bnod_eul.eul_pitc,
                    bnod_lin.linacc_x, bnod_lin.linacc_y, bnod_lin.linacc_z);
        ...
        ++cnt;
        if (cnt%20==0) {
            printf("\nmean time taken for acquiring data after %d is %f second\n\n", cnt, mean_time_taken / ((double)cnt));
        }
      }
   ...
   if(strcmp(datatype, "lin") == 0) {
    ...
    /* ------------------------------------------------------------------------------------------ *
     *  "-t imu " reads Imu collection of gyro, euler, quaternion and lin accel from the sensor.  *
     * This requires the sensor to be in fusion mode (mode > 7).                                  *
     * ------------------------------------------------------------------------------------------ */
    if(strncmp(datatype, "imu", 3) == 0) {
        int mode = get_mode();
        if(mode < 8) {
            printf("Error getting Imu, sensor mode %d is not a fusion mode.\n", mode);
            exit(-1);
        }

        struct bnogyr bnogyr_data;
        struct bnoeul bnoeul_data;
        struct bnoqua bnoqua_data;
        struct bnolin bnolin_data;
        unsigned int cnt = 0;
        double mean_time_taken = 0.;
        ...
        while(1){
            ...
            res = get_imu(&bnogyr_data, &bnoeul_data, &bnoqua_data, &bnolin_data);
            if(res != 0) {
                printf("Error: Cannot read Imu data.\n");
                exit(-1);
            }
             ...
             * print the formatted output string to stdout (Example below) *
             * ----------------------------------------------------------- */
            printf("GYR %3.2f %3.2f %3.2f; EUL %3.2f %3.2f %3.2f; QUA %3.2f %3.2f %3.2f %3.2f; LIN %3.2f %3.2f %3.2f\n",
                   bnogyr_data.gdata_x, bnogyr_data.gdata_y, bnogyr_data.gdata_z,
                   bnoeul_data.eul_head, bnoeul_data.eul_roll, bnoeul_data.eul_pitc,
                   bnoqua_data.quater_w, bnoqua_data.quater_x, bnoqua_data.quater_y, bnoqua_data.quater_z,
                   bnolin_data.linacc_x, bnolin_data.linacc_y, bnolin_data.linacc_z);

            t = clock() - t;
            double time_taken = ((double)t)/CLOCKS_PER_SEC; // in seconds
            printf("Sensor reading took %f seconds \n", time_taken);
            if(strcmp(datatype, "imu_sp") == 0) {
                usleep(200 * 1000);
            }

            /* -------------------------------------------------------- *
             *  Omit the html file for writing linear acceleration data *
             * -------------------------------------------------------- */
            mean_time_taken += time_taken;
            ++cnt;
            if (cnt%20==0) {
                printf("\nmean time taken for acquiring data after %d is %f seconds \n\n", cnt, mean_time_taken / ((double)cnt));
            }
        }
    } /* End reading Imu collection of gyro, euler, quaternion and lin accel */
