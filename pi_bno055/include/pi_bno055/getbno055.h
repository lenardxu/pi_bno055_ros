...
#ifndef PI_BNO055_GETBNO055_H
#define PI_BNO055_GETBNO055_H

#define I2CBUS               "/dev/i2c-3"
...
/* ------------------------------------------------------------ *
 * BNO055 unit factor structs. Data gets filled in based on     *
 * the query of unit selection.                                 *
 * ------------------------------------------------------------ */
struct bnoufact{
    double acc_factor;
    double gyr_factor;
...
/* ------------------------------------------------------------ *
 * Acceleration range mode, name to value translation           *
 * ------------------------------------------------------------ */
typedef enum {
    g2 = 0x00,
    g4 = 0x01,
...
/* ------------------------------------------------------------ *
 * Unit selection (units and data output format) mode           *
 * ------------------------------------------------------------ */
typedef enum {
    ros_imu_win = 0x02,  // 0b00000010
    ros_imu_andr = 0x82  // 0b10000010
} unit_sel_t;
...
extern void close_i2cbus();               // close the I2C bus
...
extern int get_ufactor(struct bnoufact*);  // read unit selections and then turn them into corresponding unit factors
extern int get_acc(struct bnoacc*);       // read accelerometer data
extern int get_acc_with_dt(struct bnoacc*, // read accelerometer data with computing the elapsed time for waiting for, reading and processing acc
                           long *elapsed_t);
extern int get_mag(struct bnomag*);       // read magnetometer data
extern int get_gyr(struct bnogyr*);       // read gyroscope data
extern int get_gyr_with_dt(struct bnogyr*, // read gyroscope data with computing the elapsed time for waiting for, reading and processing angular rate
                           long *elapsed_t);
extern int get_eul(struct bnoeul*);       // read euler orientation
extern int get_qua(struct bnoqua*);       // read quaternion data
extern int get_qua_with_dt(struct bnoqua*, // read quaternion data with computing the elapsed time for waiting for, reading and processing quaternion
                           long *elapsed_t);
extern int get_gra(struct bnogra*);       // read gravity data
extern int get_lin(struct bnolin*);       // read linear acceleration data
extern int get_imu(struct bnogyr*,
                   struct bnoeul*,
                   struct bnoqua*,
                   struct bnolin*);       // read Imu (gyroscope+euler[redundant]+quaternion+accelerometer) data required by sensor_msgs/Imu
...
extern int set_config_mode();             // set to the config operation mode
extern int set_last_mode(opmode_t);       // set to the last operation mode
extern int print_mode(int);               // print ops mode string
extern int get_unit();                    // get the sensor SI unit configuration
extern int set_unit(unit_sel_t);          // set the sensor SI unit configuration
...
extern int set_remap(char, int);          // set axis remap config or sign according to given character
